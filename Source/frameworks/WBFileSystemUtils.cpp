...
QStringList WBFileSystemUtils::sTempDirToCleanUp;


WBFileSystemUtils::WBFileSystemUtils()
{
    // NOOP
}
...
QString WBFileSystemUtils::removeLocalFilePrefix(QString input)
{
#ifdef Q_OS_WIN
...
bool WBFileSystemUtils::isAZipFile(QString &filePath)
{
...
bool WBFileSystemUtils::copyFile(const QString &source, const QString &destination, bool overwrite)
{
...
bool WBFileSystemUtils::copy(const QString &source, const QString &destination, bool overwrite)
{
...
bool WBFileSystemUtils::deleteFile(const QString &path)
{
    QFile f(path);
...
QString WBFileSystemUtils::defaultTempDirPath()
{
    return QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/" + defaultTempDirName();
}

QString WBFileSystemUtils::createTempDir(const QString& templateString, bool autoDeleteOnExit)
{
    ...
    if (autoDeleteOnExit)
        WBFileSystemUtils::sTempDirToCleanUp << dir.path();

    return dir.path();
}


QString WBFileSystemUtils::nextAvailableFileName(const QString& filename, const QString& inter)
{
    QFile f(filename);
...
void WBFileSystemUtils::deleteAllTempDirCreatedDuringSession()
{
...
void WBFileSystemUtils::cleanupGhostTempFolders(const QString& templateString)
{
...
QStringList WBFileSystemUtils::allFiles(const QString& pDirPath, bool isRecursive)
{
    QStringList result;
...
QFileInfoList WBFileSystemUtils::allElementsInDirectory(const QString& pDirPath)
{
...
bool WBFileSystemUtils::deleteDir(const QString& pDirPath)
{
    if (pDirPath == "" || pDirPath == "." || pDirPath == "..")
        return false;
...
bool WBFileSystemUtils::copyDir(const QString& pSourceDirPath, const QString& pTargetDirPath)
{
...
bool WBFileSystemUtils::moveDir(const QString& pSourceDirPath, const QString& pTargetDirPath)
{
...
QString WBFileSystemUtils::cleanName(const QString& name)
{
    QString result = name;
...
QString WBFileSystemUtils::normalizeFilePath(const QString& pFilePath)
{
    QString result = pFilePath;
    return result.replace("\\", "/");
}

QString WBFileSystemUtils::digitFileFormat(const QString& s, int digit)
{
    return s.arg(digit, 3, 10, QLatin1Char('0'));
}


QString WBFileSystemUtils::thumbnailPath(const QString& path)
{
...
QString WBFileSystemUtils::extension(const QString& fileName)
{
    QString extension("");
...
QString WBFileSystemUtils::lastPathComponent(const QString& path)
{
...
QString WBFileSystemUtils::mimeTypeFromFileName(const QString& fileName)
{
    ...
    // Apple widget
    if (ext == "wdgt") 
		return "application/vnd.apple-widget";
    if (ext == "swf") 
...
QString WBFileSystemUtils::fileExtensionFromMimeType(const QString& pMimeType)
{
    if (pMimeType == "application/msexcel") 
		return "xls";
    if (pMimeType == "application/mspowerpoint") 
		return "ppt";
    if (pMimeType == "image/ief") 
		return "ief";
    if (pMimeType == "audio/x-mpegurl") 
		return "m3u";
    if (pMimeType == "application/x-iwork-keynote-sffkeynote") 
		return "key";
    if (pMimeType == "application/vnd.oasis.opendocument.formula") 
		return "odf";
    if (pMimeType == "audio/x-aiff") 
		return "aif";
    if (pMimeType == "application/vnd.oasis.opendocument.presentation") 
		return "odp";
    if (pMimeType == "application/xml") 
		return "xml";
    if (pMimeType == "image/x-rgb") 
		return "rgb";
    if (pMimeType == "application/vnd.oasis.opendocument.spreadsheet") 
		return "ods";
    if (pMimeType == "text/rtf") 
		return "rtf";
    if (pMimeType == "application/vnd.oasis.opendocument.text") 
		return "odt";
    if (pMimeType == "image/x-xbitmap") 
		return "xbm";
    if (pMimeType == "video/x-la-asf") 
		return "lsx";
    if (pMimeType == "image/pipeg") 
		return "jfif";
    if (pMimeType == "image/x-portable-pixmap") 
		return "ppm";
    if (pMimeType == "text/csv") 
		return "csv";
    if (pMimeType == "image/x-portable-graymap") 
		return "pgm";
    if (pMimeType == "application/vnd.oasis.opendocument.chart") 
		return "odc";
    if (pMimeType == "application/vnd.oasis.opendocument.database") 
		return "odb";
    if (pMimeType == "image/x-cmx") 
		return "cmx";
    if (pMimeType == "image/x-icon") 
		return "ico";
    if (pMimeType == "audio/mpeg") 
		return "mp3";
    if (pMimeType == "audio/x-wav") 
		return "wav";
    if (pMimeType == "image/x-portable-bitmap") 
		return "pbm";
    if (pMimeType == "image/x-cmu-raster") 
		return "ras";
    if (pMimeType == "text/plain") 
		return "txt";
    if (pMimeType == "image/x-xpixmap") 
		return "xpm";
    if (pMimeType == "audio/x-pn-realaudio") 
		return "ram";
    if (pMimeType == "application/x-iwork-numbers-sffnumbers") 
		return "numbers";
    if (pMimeType == "audio/basic") 
		return "snd";
    if (pMimeType == "application/zip") 
		return "zip";
    if (pMimeType == "application/x-iwork-pages-sffpages") 
		return "pages";
    if (pMimeType == "video/x-sgi-movie") 
		return "movie";
    if (pMimeType == "image/x-xwindowdump") 
		return "xwd";
    if (pMimeType == "image/x-portable-anymap") 
		return "pnm";
    if (pMimeType == "image/cis-cod")
		return "cod";
    if (pMimeType == "application/msword") 
		return "doc";
    if (pMimeType == "text/html") 
		return "html";
    if (pMimeType == "audio/mid") 
		return "mid";
    if (pMimeType == "image/jpeg") 
		return "jpeg";
    if (pMimeType == "image/png") 
		return "png";
    if (pMimeType == "image/bmp") 
		return "bmp";
    if (pMimeType == "image/tiff") 
		return "tiff";
    if (pMimeType == "image/gif") 
		return "gif";
    if (pMimeType == "image/svg+xml") 
		return "svg";
    if (pMimeType == "application/pdf") 
		return "pdf";
    if (pMimeType == "video/quicktime") 
		return "mov";
    if (pMimeType == "video/mpeg") 
		return "mpg";
    if (pMimeType == "video/mp4") 
		return "mp4";
    if (pMimeType == "video/x-ms-asf") 
		return "asf";
    if (pMimeType == "video/x-ms-wmv") 
		return "wmv";
    if (pMimeType == "video/x-ms-wvx") 
		return "wvx";
    if (pMimeType == "video/x-ms-wm") 
		return "wm";
    if (pMimeType == "video/x-ms-wmx") 
		return "wmx";
    if (pMimeType == "video/x-msvideo") 
		return "avi";
    if (pMimeType == "video/ogg") 
		return "ogv";
    if (pMimeType == "video/x-flv") 
		return "flv";
    if (pMimeType == "video/x-m4v") 
		return "m4v";
    if (pMimeType == "application/widget") 
		return "wgt";
    if (pMimeType == "application/vnd.apple-widget") 
		return "wdgt";
    if (pMimeType == "application/x-shockwave-flash") 
		return "swf";
...
WBMimeType::Enum WBFileSystemUtils::mimeTypeFromString(const QString& typeString)
{
    WBMimeType::Enum type = WBMimeType::UNKNOWN;
   ...
   /* else if (typeString.startsWith("application/vnd.mnemis-uniboard-tool"))
    {
        type = WBMimeType::UniboardTool;
    } */

    else if (typeString.startsWith("application/wboard-tool"))
    {
        type = WBMimeType::WboardTool;
    }
    return type;

}

WBMimeType::Enum WBFileSystemUtils::mimeTypeFromUrl(const QUrl& url)
{
    return mimeTypeFromString(mimeTypeFromFileName(url.toString()));
}

QString WBFileSystemUtils::getFirstExistingFileFromList(const QString& path, const QStringList& files)
{
...
/*
bool WBFileSystemUtils::compressDirInZip(const QDir& pDir, const QString& pDestPath, QuaZipFile *pOutZipFile, bool pRootDocumentFolder, WBProcessingProgressListener* progressListener)
{
...
*/
bool WBFileSystemUtils::expandZipToDir(const QFile& pZipFile, const QDir& pTargetDir)
{
    /*
...
QString WBFileSystemUtils::md5InHex(const QByteArray &pByteArray)
{
    /*
    MD5_CTX ctx;
    MD5_Init(&ctx);
...
QString WBFileSystemUtils::md5(const QByteArray &pByteArray)
...
QString WBFileSystemUtils::readTextFile(QString path)
{
    QFile file(path);
